...
By chance, the data plot will be detected with probability, $1/12=0.083$. This corresponds to about 2 detections out of 20, generally this would be $n/12$ detections. Let's suppose the alternative hypothesis has probability of detection $p_A (>1/12)$. If we expect the data to be detected twice as often as by chance, $p_A=1/6$. To determine the probability of observing $n/12$ detections, given $p_A$, we can use a binomial distribution, where $k=ceiling(n/12)$, to calculate the power:

$$P(X>k|p=p_A,n) = {n \choose k} p_A^k (1-p_A)^{n-k}$$

If $p_A=1/6$ we would need almost 100 evaluations for each lineup to expect at least $n/12$ detections. With $p_A=0.25$ approximately 50 evaluations should yield sufficient detections. We will use a pilot study to estimate $p_A$. 
...
n <- 20:100
k <- ceiling(n/12)
pA <- seq(1/12, 1/2, 0.01)
g <- expand.grid(n, k, pA)
pow <- tibble(n=g[,1], k=g[,2], pA=g[,3]) 
pow <- pow %>% 
  mutate(prob = pbinom(k, n, pA, lower.tail=FALSE)) 
ggplot(pow, aes(x=n, y=pA, colour=prob, group=pA)) + 
  geom_point() +
