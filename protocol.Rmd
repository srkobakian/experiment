...
1. Show that spatial disease trends, that impact highly populated small areas, are detected with higher accuracy when viewed in a hexagon tile map display.
...
A correct result occurs when the chosen plot matches the position of the real plot, this will be recorded in an additional binary variable called $collect$.

The accuracy of the choices for each display will be calculated as a proportion: 
$accuracy = number correct/number shown$.


### Efficiency

High efficiency occurs when a small amount of time is taken to evaluate each lineup.
This will be measured as the inverse of the numeric variable measuring the length of time taken to submit the answers to the evaluation of each line up.

The efficiency of the choices for each display will be calculated by the time taken as a fraction of the maximum time taken by all respondents to respond:
$efficiency = time taken/maximumtime taken$.
...
Design:
We will model the difference between the accuracy and efficiency when evaluating hexagon tile maps and geographic maps.
...
Interactions:
Design and Difficulty: The interaction between the perceived difficulty of evaluating hexagon tile maps and geographic maps will be included to measure whether particiants feel differently about the designs.

Spatial Trend and Difficulty: The interaction between the spatial trend used in the lineup of hexagon tile maps and geographic maps will be included to measure whether particiants feel differently about finding different spatial trends.
