...
ggplot(d %>% mutate(Detected = factor(detect, levels = c(0,1), 
  labels = c("Detected? No", "Detected? Yes")),
  `Map Type` = type)) + 
  geom_bar(aes(x = fct_infreq(reason), fill = `Map Type`)) +  
  facet_grid(trend ~ Detected) +  
...
The estimated marginal mean is the fitted value from a model over the treatment effects. The treatment effects considered in this model are map type displayed, trend model included, and the replicate. The `emmeans` package is used to calculate the estimated marginal mean.
The value, between 0 and 1, can be interpretted as the probability of detecting the real data plot in the lineup, for each combination of map type, trend model and replicate. 
