...
d %>% 
  ...
  knitr::kable(., format = "latex", booktabs = TRUE, 
    ...
    caption = "The experimental design results in the following allocation of replicate data sets within each trend model to groups A and B.") %>% kable_styling("striped") %>% 
...
```{r detection-compare, fig.cap = "The detection rates achieved by participants are contrasted when viewing the four replicates of the three trend models. Each point shows the probability of detection for the lineup display, the facets separate the trend models hidden in the lineup. The points for the same data set shown in a choroleth or haxgon tile map display are linked to show the difference in the detection rate."}
...
d %>% group_by(type, trend) %>%
  summarise(m = round(mean(detect),3),
    std.dev = paste0("(", as.character(round(sd(detect),3)),")")) %>% 
  ...
  knitr::kable(., format = "latex", align = "lccc", booktabs = TRUE, caption = "The rate of detection for each trend model has been calculate for the choropleth and hexagon tile map displays. The associated standard errors are also included.")
```

Table \ref{detect-glm1} is a summary of the generalised linear mixed effects model.
The estimate of the intercept represents the detection rate for the Choropleth display of the NW-SE trend. 
...
tidy(glmer1) %>%
  ...
  select(Term = term, 
    ...
    `Std. Error` = std.error, 
    `P value` = p.value) %>% 
  knitr::kable(format = "latex", escape = FALSE, align= "rclrr", booktabs = T,
...
The model summary in Table \ref{tab:detect-glmer1} shows that the predicted log odds of the detection rate is 0.46 times that of the choropleth lineups, for the lineups of hexagon tile map. 
The baseline trend model used is the NW-SE trend, the detection rate is expected to decrease comparatively for choropleth displays of the Three Cities and All Cities trends. However, when the interaction is considered the detection rate is expected to increase for the Three Cities and All Cities trends shows in the Hexagon tile map display.
...
The relationship between time taken to submit, and the correct detection by the participant of the real data display in a lineup is considered in Figure \ref{fig:hist-height}. The bimodal distributions are similar across the type of display and the trend model hidden in the real data plot.
...
```{r hist-fill,  eval=FALSE, fig.cap = "The time taken to evaluate each display is broken into five second windows. The height of the histogram bars show the proportion of evaulations that were submitted within each time window."}
ggplot(d %>% rename(Detected = detect_f), 
  aes(x = time_taken, fill = Detected, group = Detected)) +  
  scale_fill_manual(values = detect_colours) +
...
```{r time-lmer1, eval = FALSE, results="asis"}
# Mixed effects models
d %>% mutate(ln_time = time_taken) %>% 
time_lmer1 <- glmer(ln_time ~ type*trend + (1|contributor),data = .)

lmer_terms <- c("Intercept", "Hex.", "Three Cities", "All Cities", 
  "Hex:Three Cities", "Hex:All Cities")
...
d %>% 
  ...
  ungroup() %>% 
  knitr::kable(., format = "latex", booktabs = TRUE,
    caption = "The amount of participants that selected each reason for their choice of plot when looking at each trend model shown in Choropleth and Hexagon Tile maps. The facets show whether or not the choice was correct.") %>% kable_styling("striped") 
#%>% 
