...
Plots will be made in R [@R], with the \pkg{ggplot2} package [@ggplot2].   
   
   
### Modelling

The likelihood of detecting the data plot in the lineup can be modelled using a linear mixed effects model. 
The R [@RCore] `glmer()` function in the \pkg{lme4} [@lme4] package implements generalised linear mixed effect models. The model used includes the two main effects map type and trend model, which gives the fixed effects model to be:
