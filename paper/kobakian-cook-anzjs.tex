...
test where a plot is treated as a test statistic. If the data plot is
selected it is analogous to a rejection of the null hypothesis (no
structure), and the observed data plot is unlikely to arise from the
null scenario.
...
to how they are seen from the choropleth.
...
outlined in \citet{GTPCCD}, are followed.
...
answer. Only 1 reported having lived in Australia.
...
\caption{\label{tbl-desc-stats}The mean and standard deviation of the
rate of detection for each trend model, calculated for the choropleth
and hexagon tile map displays.}
...
the Three Cities trend, 0.03 is extremely low, especially compared to
the NW-SE trend of 0.52. When the All Cities trend is presented in a
...
\caption{\label{tbl-reason}The main reason that participants wrote for
their choice of plot when looking at each trend model shown in
...
Add Election results data + plus other statistics, other hexagon layout
software
